...
server <- function(input, output, session) {
  ...
  observeEvent(input$round_choice, {
    region_choices <- mmHist %>% filter(Round == input$round_choice) |>
      distinct(`Region Name`) |> pull(`Region Name`)
  ...
  output$Variable_Description <- renderPrint({
    if (input$stat_type == "Defensive and Opponent Stats"){
      cat(" Defensive and Opponent Stats:\n",
        "----------------------------------------------\n",
        "Team:        | Team name\n",
        ...
        "`2P%D`:      | Two-Point Defense\n",
        "`3P%D`:      | Three-Point Defense\n",
        "`3PRD`:      | Three-Point Rate Defense\n",
  ...
  output$history_plot <- renderPlotly({
    plt <- mmHistPlot() %>%
      ...
      labs(title = paste("NCAA Historic Tournament Results for...", "\bRound:", input$round_choice, "&","**Region**:", input$region_choice),
           x = "Year",
           y = "Winning Score") +
